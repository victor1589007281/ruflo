...
## 2026-07-26 收尾轮：一个"代价大于收益"的错误判断 + 一个偶发失败挖出的两个真竞争

### ① 全仓 59 个调用点把团队名传进了叫 `run_id` 的字段

上一节的 E2E 已证明 llm.jsonl / trace spans / rewards 三源可按 run_id 对账，但
**team.jsonl 对不上**。我当时写的理由是"代价大于收益、留作单独一轮"——**那个理由错了**：
它默认了"团队身份只能靠 labels 携带"。`MetricEvent.RunID` 从一开始就与 `Labels` 分离且
不进 Prometheus；给它加一个同样不进 Prometheus 的 `Team` 字段，三件事同时成立——run_id
恢复本义 / 团队身份不丢 / **一个 Prometheus 序列都没动**。

真实范围也比原记录大：不是"20+ 处"，而是**全仓 59 个 `RecordRun` 无一例外**。RunID 不靠
穿 ctx 拿（只有约一半调用点的外层函数带 ctx）——`team.LastRunID` 里已是同一个值，于是把
(team → RunID, 团队名) 收到 `recordTeamRun` 一处。拿不到 RunID 时**留空而不回填团队名**：
回填只是把缺陷换个位置。

**真集群已验（36/36 那轮）**：`FULL4=1`（`run-e2e-graph-…` 同时贯穿 llm/team/rewards/spans）·
`TEAMROWS=29 TEAMFIELD=29`（团队身份全带）· `TEAMLABEL=0`（一条都没溜进 labels）·
`BADRUN=0`（无团队名冒充 run_id）。三条已固化成 E2E 断言——**跨 sink 的 join 单测证不了**：
每个 sink 的 run_id 都是各自代码路径独立写出来的。

### ② 一个偶发失败，用对工具立刻现形

上一轮我诚实登记了"观察到一次 tests/eval 偶发失败、8 次未复现"。用 `-race` 跑同一个包：
**3 条 DATA RACE + 2 个子测试失败**。偶发不是玄学——那 8 次复现尝试全没带 `-race`。

- **`pkg/memory` 真生产缺陷**：`Add` 对高权重记忆 `go PersistToDisk()`，而它只在 RLock 里
  拷**指针切片**、在**锁外**序列化；同时 `Retrieve` 在写锁内 `Touch()` 改同一对象。
  后果不止报警：`time.Time` 是多字长值，**撕裂读能把无意义时间戳写进磁盘**，下次
  `loadFromDisk` 拿它算 `Retention()` ⇒ 一条记忆可能被判成早该遗忘而**丢弃**。
  修法是按值快照后锁外编码（不把序列化搬进读锁——那会让记忆检索被写盘卡住）。
- **我自己上一轮的"修复"是个空承诺**：`WaitBackground` 依赖 `bg`，但 `ForceDream` 起的
  **外层 goroutine 从未登记** ⇒ `bg.Wait()` 在计数器为 0 时立刻返回，随后内层 `Add(1)`
  与已开始的 `Wait` 并发（违反 WaitGroup 契约）。两个后果叠加：race 报警，**且它本要修的
  "靠睡眠等后台"根本没修好**——一个看起来已修的修复。
- 两个**此前零测试**的包（`pkg/memory` / `pkg/dreaming`）补上第一批测试，只对着缺陷本身。

**方法论补一条**：`-race` 之于并发缺陷，等价于变异反证之于"测试有没有牙"——不带它跑一百遍
也可能全绿。memory 那条测试**只有带 `-race` 才有牙**，已写进测试注释，免得将来有人把 CI 的
`-race` 去掉后以为覆盖没变。
