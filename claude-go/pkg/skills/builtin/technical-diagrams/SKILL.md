...
description: 为技术文档制作彩色、可读、忠于源码的架构图与流程图（mermaid 语义配色八色板 + 手绘内联 SVG 主图 + 窄栏可读性 + 渲染实测验收）。适用于：需要画或改架构图、分层图、流程图、状态机、时序图、部署拓扑；给现有灰白 mermaid 上语义配色；修"图太宽看不清"；往手册/README/wiki/文档站加图；按功能系统重组文档目录并保证编号不漂移。即使没明说"图表"，只要任务是"写技术文档并配图""让文档不那么简陋""把架构讲清楚""参考业界最佳实践画图"，也应使用本技能。
...
三步都要做，可直接照抄的脚本见同目录 `references/verify-scripts.md`：
...
画完架构图后，常会发现**目录组织与架构图不一致**——图上是清晰的五层，目录里却是
"专题章"与功能章互相重叠。让目录直接镜像架构图的功能系统（一层一章），读者才能从图跳到章。

改编号是**机械活，必须脚本化**，手改必漏。编号藏在四处，缺一处就漂移：

| 位置 | 改法 |
|---|---|
| 目录清单（manifest/SUMMARY/mkdocs.yml） | 重写 parent / sort / title |
| 每页 `<h1>` | 整体替换成新标题 |
| 页内 `<h2>/<h3>` 编号前缀 | 页内编号**一律以本页编号打头**，故是纯前缀替换：`10.7.*` → `4.8.*` |
| 跨章引用 | 见下 |

跨章引用不要手工列 (old,new) 对照——**引用几乎都写成
`<a href=".../SLUG">第7章</a>`，标签里的编号可由目标 slug 的新编号确定性推出**，
遍历全部链接重写标签即可，一次覆盖全站。剩下的明文引用（不带链接的）用
"剥掉所有 `<a>`/`<h>` 后再 grep 编号"筛出来，逐条人工确认——注意区分
**指向设计文档的 `design/01 §4.9` 不能改**。

三条硬约束：

1. **slug/文件名保持不变**，只改编号与归属——旧链接与书签不许失效
2. 章号变了，章首"本章覆盖…"的子节清单必须同步重写（脚本改不了这个，逐页人工过）
3. 收尾必须机械自检：每页 h1 == 目录标题、页内编号归属本页、子页章号 == 父页章号

被拆散的旧章不要删——改写成**「目录改版对照表」附录**，登记旧→新全表。
它同时是给带书签的读者的落地页，也是改版理由的留痕。
