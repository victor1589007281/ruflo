...
// processBus 进程级默认总线 (design/02 §3.4.2 的 "进程内 channel (默认)" 形态)。
//
// 为什么需要一个进程单例, 而不是把 *ChanBus 从装配处一路传下去: 事件的**产生方**
// 常常在依赖链的最底层 (如 pkg/api 的重试/熔断路径), 而**订阅方**在最上层
// (飞书播报/面板)。逐层传参要穿过十几个既有构造函数, 每加一个产生方就要再穿一遍;
// 更糟的是漏传不会编译报错, 只会让那一路事件静默消失 —— 这正是 OnLLMEvent
// 单回调字段踩过的坑 (7 个生产 api.Client 里只有 1 个被赋值, 另外 6 个的
// 429/熔断/致命失败从来没人收到)。
//
// 单例只是"chan 后端"的默认装配; T2/T3 形态换 NATS 时替换的是这一处的实现,
// 订阅/发布两侧的代码一字不动。
var (
...
// Default 返回进程级默认总线 (懒创建, 并发安全)。
func Default() *ChanBus {
	processOnce.Do(func() { processBus = NewChanBus(DefaultBuf) })
	return processBus
}
